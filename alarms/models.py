...
class Alarms(models.Model):
    ...
    electric_parameter = models.ForeignKey(ElectricParameters, on_delete=models.PROTECT)
    max_value = models.DecimalField(blank=True, null=True, max_digits=20, decimal_places=6)
    min_value = models.DecimalField(blank=True, null=True, max_digits=20, decimal_places=6)
...
class AlarmEvents(models.Model):
    ...
    def __unicode__(self):
        return  self.alarm.alarm_identifier + " - " + str(self.value) + \
                self.alarm.electric_parameter.name + " - " + str(self.triggered_time)
