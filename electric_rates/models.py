...
class Holydays(models.Model):
    ...
    description = models.CharField(u"Descripción", max_length=128, blank=True,
                                   null=True)
...
class ElectricRatesDetail(models.Model):
    ...
    electric_rate = models.ForeignKey(ElectricRates, on_delete=models.PROTECT)
    KDF = models.DecimalField("Cargo por kilowatt de demanda facturable",
                              max_digits=12, decimal_places=6)
    KWHP = models.DecimalField(u"Cargo por kilowatt - hora de energía de punta",
                               max_digits=12, decimal_places=6)
    KWHI = models.DecimalField(
        u"Cargo por kilowatt - hora de energía intermedia", max_digits=12,
        decimal_places=6)
    KWHB = models.DecimalField(u"Cargo por kilowatt - hora de energía de base",
                               max_digits=12, decimal_places=6)
    FRI = models.DecimalField(max_digits=12, decimal_places=6)
    FRB = models.DecimalField(max_digits=12, decimal_places=6)
    KWDMM = models.DecimalField(u"Cargo por kilowatt de demanda máxima medida",
                                max_digits=12, decimal_places=6)
    KWHEC = models.DecimalField(
        u"Cargo por kilowatt - hora de energía consumida", max_digits=12,
        decimal_places=6)
...
class ElectricRatesPeriods(models.Model):
    ...
    region = models.ForeignKey(Region, on_delete=models.PROTECT, null=True,
                               blank=True)
    date_interval = models.ForeignKey(DateIntervals, on_delete=models.PROTECT,
                                      verbose_name="Periodo")
    groupdays = models.ForeignKey(Groupdays, on_delete=models.PROTECT,
    ...
    period_type = models.CharField("Tipo de periodo", max_length=10,
                                   choices=PERIODS, null=True, blank=True)
...
class DACElectricRateDetail(models.Model):
    region = models.ForeignKey(Region, on_delete=models.PROTECT, null=True,
                               blank=True)
    date_interval = models.ForeignKey(DateIntervals, on_delete=models.PROTECT,
                                      verbose_name="Periodo", null=True,
                                      blank=True)
    month_rate = models.DecimalField(u"Cargo mensual", max_digits=12,
                                     decimal_places=6)
    kwh_rate = models.DecimalField(u"Cargo por kilowatt - hora", max_digits=12,
                                   decimal_places=6)
    date_init = models.DateField("Fecha de Inicio")
    date_end = models.DateField("Fecha de Fin")
...
class ThreeElectricRateDetail(models.Model):
    kw_rate = models.DecimalField(u"Cargo Demanda ", max_digits=12,
        decimal_places=6)
    kwh_rate = models.DecimalField(u"Cargo por kilowatt - hora", max_digits=12,
        decimal_places=6)
    date_init = models.DateField("Fecha de Inicio")
    ...
    def __unicode__(self):
        return "Cuota Aplicable del " + str(self.date_init) + " al " + str(
            self.date_end)
